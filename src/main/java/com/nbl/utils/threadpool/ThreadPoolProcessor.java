package com.nbl.utils.threadpool;
...
public class ThreadPoolProcessor {
	...
	public static ThreadPoolProcessor getInstanceScheduled() {
		if (poolScheduled == null) {
			synchronized (ThreadPoolProcessor.class) {
				if (poolScheduled == null) {
					poolScheduled = new ThreadPoolProcessor(ExecutorServiceFactory.getInstance().createScheduledThreadPool());
				}
			}
		}
	...
	public static ThreadPoolProcessor getInstanceSingle(ExecutorService executor) {
		if (poolSingle == null) {
			synchronized (ThreadPoolProcessor.class) {
				if (poolSingle == null) {
					poolSingle = new ThreadPoolProcessor(ExecutorServiceFactory.getInstance().createSingleThreadExecutor());
				}
			}
		}
		return poolSingle;
	...
	public static ThreadPoolProcessor getInstanceFixed(int threadPoolMax) {
		if (poolFixed == null) {
			synchronized (ThreadPoolProcessor.class) {
				if (poolFixed == null) {
					poolFixed = new ThreadPoolProcessor(ExecutorServiceFactory.getInstance().createFixedThreadPool(threadPoolMax));
				}
			}
		}
		return poolFixed;
